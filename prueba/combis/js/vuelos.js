var app = new function() {
        ...
        this.vuelos = [vuelo1, vuelo2, vuelo3, vuelo4, vuelo5 , vuelo6];
        ...
        this.mostrarVuelos = function() {
          ...
          if (this.vuelos.length > 0) {
            for (i = 0; i < this.vuelos.length; i++) {
              ...
              data += '<tr>';
              data += '<td>CHOMBY # '+ (i+1) + ' ORIGEN: ' + this.vuelos[i].origen + ', DESTINO: ' + this.vuelos[i].destino + ', SALIDA: ' + this.vuelos[i].hora.toDateString() + " " + hora + ":" + minutos + '</td>';
        ...
        this.Reservar = function (item) {
          ...
          document.getElementById('datosvuelo').innerHTML = "Chomby # " + (item + 1) + ":<br>ORIGEN: " + this.vuelos[item].origen + '<br>DESTINO: ' + this.vuelos[item].destino + '<br>SALIDA: ' + this.vuelos[item].hora.toDateString() + " " + hora + ":" + minutos + '<br>PRECIO BASE: $' + this.vuelos[item].costobase ;
        ...
        this.consultarReserva = function () {
          ...
          if (isNaN(d) || d == 0) {
            ...
            var data = '<br>CHOMBY RESERVADOS DE ' + d;
            for (i = 0; i < this.vuelos.length; i++) {
              ...
              if (auxDoc != -1) {
                ...
                var minutos = this.vuelos[i].hora.getMinutes() < 10 ? '0' + this.vuelos[i].hora.getMinutes() : this.vuelos[i].hora.getMinutes();
                data += '<tr>';
                data += '<td>Combi # '+ (i+1) + "= ORIGEN: " + this.vuelos[i].origen + ', DESTINO: ' + this.vuelos[i].destino + ', SALIDA: ' + this.vuelos[i].hora.toDateString() + " " + hora + ":" + minutos + '</td>';
                data += '</tr>';
              }
            }
